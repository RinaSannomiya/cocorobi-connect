...
const ComprehensiveSurvey = () => {
  ...
  const { saveComprehensiveSurveyData } = useAuth();
  ...
  const hasInputData = () => {
    // 基本的なフォームデータをチェック
    if (cardCount) return true;
    if (usesCardApp !== null) return true;
    if (cardAppType.length > 0) return true;
    if (otherCardApp.trim()) return true;
    if (isEightPremium !== null) return true;
    if (registeredCardCount) return true;
    if (currentCompany.trim()) return true;
    if (currentPosition.trim()) return true;
    if (isIndividualBusiness) return true;
    if (pastCompanies.some(company => company.name.trim() || company.position.trim())) return true;
    if (linkedinUrl.trim()) return true;
    if (facebookUrl.trim()) return true;
    
    return false;
  };

  // ブラウザバック対策（入力データがある場合のみ確認ダイアログ表示）
  useEffect(() => {
    const handlePopState = (e: PopStateEvent) => {
      // 入力データがある場合のみ確認
      if (hasInputData()) {
        const shouldLeave = window.confirm(
          '入力内容が失われますが、前のページに戻りますか？'
        );
  ...
  }, [cardCount, usesCardApp, cardAppType, otherCardApp, isEightPremium, registeredCardCount, currentCompany, currentPosition, isIndividualBusiness, pastCompanies, linkedinUrl, facebookUrl]);
  ...
  const validateForm = () => {
    ...
    pastCompanies.forEach(company => {
      ...
      if (company.name.trim() === '' && company.position.trim() === '') {
        // 空の場合は企業名を必須とする
        errors[`pastCompany_${company.id}_name`] = '企業名を入力してください';
      }
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'アンケートの保存に失敗しました';
    ...
    <div className="min-h-screen bg-gray-50 pt-20 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">本登録</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        {isEightPremium === true && (
                          <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-md">
                            <div className="flex items-start space-x-2">
                              <CheckCircle className="h-5 w-5 text-[#00736d] mt-0.5 flex-shrink-0" />
          ...
          <div className="flex justify-center">
            <button
              type="submit"
              disabled={isLoading}
              className="flex items-center justify-center py-3 px-8 border border-transparent text-base font-medium rounded-md text-white bg-[#1D73C3] hover:bg-[#155a9a] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#1D73C3] disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
            >
              {isLoading ? (
                <>
...
export default ComprehensiveSurvey;